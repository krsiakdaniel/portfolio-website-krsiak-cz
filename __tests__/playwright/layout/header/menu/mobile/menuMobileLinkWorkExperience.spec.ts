...
test.describe('Menu Mobile - Link "Work Experience"', () => {
  ...
  test('should navigate to Work Experience page correctly from the mobile menu', async () => {
    // Click on the menu toggle to open the mobile menu
    await page.click(getDataTestId(DATA_TEST_IDS.menu.toggle.component))

    // Check if the Work Experience link is visible
    const isLinkVisible = await page.isVisible(getDataTestId(DATA_TEST_IDS.menu.mobile.links.workExperience))
    expect(isLinkVisible).toBe(true)

    // Click on the Work Experience link
