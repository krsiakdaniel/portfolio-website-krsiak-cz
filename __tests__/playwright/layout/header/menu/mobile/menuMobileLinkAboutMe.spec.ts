...
test.describe('Menu Mobile - Link "About Me"', () => {
  ...
  test('should navigate to About Me page correctly from the mobile menu', async () => {
    // Click on the menu toggle to open the mobile menu
    await page.click(getDataTestId(DATA_TEST_IDS.menu.toggle.component))

    // Check if the About Me link is visible
    const isLinkVisible = await page.isVisible(getDataTestId(DATA_TEST_IDS.menu.mobile.links.aboutMe))
    expect(isLinkVisible).toBe(true)

    // Click on the About Me link
    await page.click(getDataTestId(DATA_TEST_IDS.menu.mobile.links.aboutMe))
