...
test.describe('Header - Desktop Menu - Work Experience Link', () => {
  test('should navigate to Work Experience page correctly', async ({ page }) => {
    ...
    await test.step('Check if the link goes to the work experience page when clicked', async () => {
      // Wait for the element to be ready and click it
      await page.waitForSelector(getDataTestId(DATA_TEST_IDS.menu.desktop.links.workExperience))
